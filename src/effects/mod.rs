...
use tachyonfx::{fx, Effect as TEffect, EffectTimer as TEffectTimer,
                Interpolation as TInterpolation, Motion as TMotion,
                CellFilter as TCellFilter};
...
impl EffectTimer {
    ...
    pub fn new(duration_ms: u64, interpolation: Option<Interpolation>) -> Self {
        Self { duration_ms, interpolation: interpolation.unwrap_or(Interpolation::Linear) }
    }
    #[getter] pub fn duration_ms(&self) -> u64 { self.duration_ms }
    #[getter] pub fn interpolation(&self) -> Interpolation { self.interpolation.clone() }
    fn __repr__(&self) -> String { format!("EffectTimer({}ms, {:?})", self.duration_ms, self.interpolation) }
}
...
pub(crate) enum CellFilterKind {
    All, Text,
    FgColor(RColor), BgColor(RColor),
    Inner(u16, u16), Outer(u16, u16),
    AllOf(Vec<CellFilter>), AnyOf(Vec<CellFilter>),
}
...
impl CellFilter {
    pub(crate) fn to_tachyonfx(&self) -> TCellFilter {
        match &self.inner {
            ...
            CellFilterKind::Inner(h,v) => TCellFilter::Inner(RMargin { horizontal: *h, vertical: *v }),
            CellFilterKind::Outer(h,v) => TCellFilter::Outer(RMargin { horizontal: *h, vertical: *v }),
...
#[pymethods]
impl CellFilter {
    #[staticmethod] pub fn all()  -> CellFilter { CellFilter { inner: CellFilterKind::All } }
    #[staticmethod] pub fn text() -> CellFilter { CellFilter { inner: CellFilterKind::Text } }
    #[staticmethod] pub fn fg_color(color: &Color) -> CellFilter {
        CellFilter { inner: CellFilterKind::FgColor(color.inner) }
    }
    #[staticmethod] pub fn bg_color(color: &Color) -> CellFilter {
        CellFilter { inner: CellFilterKind::BgColor(color.inner) }
    }
    #[staticmethod]
    #[pyo3(signature = (horizontal=1, vertical=1))]
    pub fn inner(horizontal: u16, vertical: u16) -> CellFilter {
        CellFilter { inner: CellFilterKind::Inner(horizontal, vertical) }
    }
    #[staticmethod]
    #[pyo3(signature = (horizontal=1, vertical=1))]
    pub fn outer(horizontal: u16, vertical: u16) -> CellFilter {
        CellFilter { inner: CellFilterKind::Outer(horizontal, vertical) }
    }
    #[staticmethod]
    pub fn all_of(filters: Vec<PyRef<CellFilter>>) -> CellFilter {
        CellFilter { inner: CellFilterKind::AllOf(filters.iter().map(|f| (**f).clone()).collect()) }
    }
    #[staticmethod]
    pub fn any_of(filters: Vec<PyRef<CellFilter>>) -> CellFilter {
        CellFilter { inner: CellFilterKind::AnyOf(filters.iter().map(|f| (**f).clone()).collect()) }
    }
    fn __repr__(&self) -> String { format!("CellFilter({:?})", self.inner) }
}
...
impl Effect {
    ...
    pub fn fade_from(from_bg: &Color, from_fg: &Color, duration_ms: u64, interpolation: Option<Interpolation>) -> Effect {
        Effect { inner: fx::fade_from(from_bg.inner, from_fg.inner, make_timer(duration_ms, interpolation)) }
    }
    #[staticmethod]
    #[pyo3(signature = (from_color, duration_ms, interpolation=None))]
    pub fn fade_from_fg(from_color: &Color, duration_ms: u64, interpolation: Option<Interpolation>) -> Effect {
        Effect { inner: fx::fade_from_fg(from_color.inner, make_timer(duration_ms, interpolation)) }
    }
    #[staticmethod]
    #[pyo3(signature = (to_bg, to_fg, duration_ms, interpolation=None))]
    pub fn fade_to(to_bg: &Color, to_fg: &Color, duration_ms: u64, interpolation: Option<Interpolation>) -> Effect {
        Effect { inner: fx::fade_to(to_bg.inner, to_fg.inner, make_timer(duration_ms, interpolation)) }
    }
    #[staticmethod]
    #[pyo3(signature = (to_color, duration_ms, interpolation=None))]
    pub fn fade_to_fg(to_color: &Color, duration_ms: u64, interpolation: Option<Interpolation>) -> Effect {
        Effect { inner: fx::fade_to_fg(to_color.inner, make_timer(duration_ms, interpolation)) }
    }
    // Text materialization
    #[staticmethod]
    #[pyo3(signature = (duration_ms, interpolation=None))]
    pub fn coalesce(duration_ms: u64, interpolation: Option<Interpolation>) -> Effect {
        Effect { inner: fx::coalesce(make_timer(duration_ms, interpolation)) }
    }
    #[staticmethod]
    #[pyo3(signature = (duration_ms, interpolation=None))]
    pub fn dissolve(duration_ms: u64, interpolation: Option<Interpolation>) -> Effect {
        Effect { inner: fx::dissolve(make_timer(duration_ms, interpolation)) }
    }
    ...
    pub fn slide_in(direction: &Motion, begin_sweep: u16, end_sweep: u16, color: Option<&Color>,
                    duration_ms: u64, interpolation: Option<Interpolation>) -> Effect {
        let c = color.map(|c| c.inner).unwrap_or(RColor::Black);
        Effect { inner: fx::slide_in(direction.to_tachyonfx(), begin_sweep, end_sweep, c, make_timer(duration_ms, interpolation)) }
    }
    #[staticmethod]
    #[pyo3(signature = (direction, begin_sweep=0, end_sweep=0, color=None, duration_ms=500, interpolation=None))]
    pub fn slide_out(direction: &Motion, begin_sweep: u16, end_sweep: u16, color: Option<&Color>,
                     duration_ms: u64, interpolation: Option<Interpolation>) -> Effect {
        let c = color.map(|c| c.inner).unwrap_or(RColor::Black);
        Effect { inner: fx::slide_out(direction.to_tachyonfx(), begin_sweep, end_sweep, c, make_timer(duration_ms, interpolation)) }
    }
    #[staticmethod]
    #[pyo3(signature = (direction, sweep_span=15, gradient_len=0, color=None, duration_ms=600, interpolation=None))]
    pub fn sweep_in(direction: &Motion, sweep_span: u16, gradient_len: u16, color: Option<&Color>,
                    duration_ms: u64, interpolation: Option<Interpolation>) -> Effect {
        let c = color.map(|c| c.inner).unwrap_or(RColor::Black);
        Effect { inner: fx::sweep_in(direction.to_tachyonfx(), sweep_span, gradient_len, c, make_timer(duration_ms, interpolation)) }
    }
    #[staticmethod]
    #[pyo3(signature = (direction, sweep_span=15, gradient_len=0, color=None, duration_ms=600, interpolation=None))]
    pub fn sweep_out(direction: &Motion, sweep_span: u16, gradient_len: u16, color: Option<&Color>,
                     duration_ms: u64, interpolation: Option<Interpolation>) -> Effect {
        let c = color.map(|c| c.inner).unwrap_or(RColor::Black);
        Effect { inner: fx::sweep_out(direction.to_tachyonfx(), sweep_span, gradient_len, c, make_timer(duration_ms, interpolation)) }
    }
    // Timing
    #[staticmethod]
    pub fn sleep(duration_ms: u64) -> Effect {
        Effect { inner: fx::sleep(duration_ms.min(u32::MAX as u64) as u32) }
    }
    // Composition
    #[staticmethod]
    pub fn sequence(effects: Vec<PyRefMut<Effect>>) -> Effect {
        let v: Vec<TEffect> = effects.into_iter()
            .map(|mut e| std::mem::replace(&mut e.inner, fx::sleep(0u32))).collect();
        Effect { inner: if v.is_empty() { fx::sleep(0u32) } else { fx::sequence(v.as_slice()) } }
    }
    #[staticmethod]
    pub fn parallel(effects: Vec<PyRefMut<Effect>>) -> Effect {
        let v: Vec<TEffect> = effects.into_iter()
            .map(|mut e| std::mem::replace(&mut e.inner, fx::sleep(0u32))).collect();
        Effect { inner: if v.is_empty() { fx::sleep(0u32) } else { fx::parallel(v.as_slice()) } }
    }
    #[staticmethod]
    ...
    pub fn repeat(mut effect: PyRefMut<Effect>, times: i32) -> Effect {
        ...
        let mode = if times < 0 { RepeatMode::Forever } else { RepeatMode::Times(times as u32) };
        Effect { inner: fx::repeat(inner, mode) }
    }
    #[staticmethod]
    pub fn ping_pong(mut effect: PyRefMut<Effect>) -> Effect {
        let inner = std::mem::replace(&mut effect.inner, fx::sleep(0u32));
        Effect { inner: fx::ping_pong(inner) }
    }
    #[staticmethod]
    pub fn never_complete(mut effect: PyRefMut<Effect>) -> Effect {
        let inner = std::mem::replace(&mut effect.inner, fx::sleep(0u32));
        Effect { inner: fx::never_complete(inner) }
    }
    ...
    fn __repr__(&self) -> String { format!("Effect(done={})", self.inner.done()) }
}
...
impl EffectManager {
    #[new]
    pub fn new() -> Self { Self { effects: Vec::new(), named: HashMap::new() } }
    ...
    pub fn has_active(&self)   -> bool  { !self.effects.is_empty() }
    ...
    fn __repr__(&self) -> String { format!("EffectManager(active={})", self.effects.len()) }
}
...
pub fn compile_effect(expression: &str) -> PyResult<Effect> {
    ...
    let effect = EffectDsl::new().compiler().compile(expression)
