...
use ratatui::widgets::{
    ...
    ScrollbarOrientation as RScrollbarOrientation,
};
...
impl ScrollbarState {
    #[new]
    pub fn new() -> Self { Self { inner: RScrollbarState::default(), pos: 0 } }
    ...
    pub fn scroll_next(&mut self) { self.inner.next(); self.pos = self.pos.saturating_add(1); }
    pub fn scroll_prev(&mut self) { self.inner.prev(); self.pos = self.pos.saturating_sub(1); }
    pub fn first(&mut self)  { self.inner.first();  self.pos = 0; }
    pub fn last(&mut self)   { self.inner.last(); }

    #[getter]
    pub fn get_position(&self) -> usize { self.pos }

    fn __repr__(&self) -> String { format!("ScrollbarState(pos={})", self.pos) }
}
...
impl Scrollbar {
    pub(crate) fn to_ratatui(&self) -> RScrollbar<'static> {
        ...
        if let Some(ref st) = self.thumb_style { s = s.thumb_style(st.inner); }
        if let Some(ref st) = self.track_style { s = s.track_style(st.inner); }
        if let Some(ref st) = self.begin_style { s = s.begin_style(st.inner); }
        if let Some(ref st) = self.end_style   { s = s.end_style(st.inner); }
        s
    }
}
...
impl Scrollbar {
    ...
    pub fn new(orientation: Option<&ScrollbarOrientation>) -> Self {
        Self {
            orientation: orientation.cloned().unwrap_or(ScrollbarOrientation::VerticalRight),
            thumb_style: None, track_style: None,
            begin_style: None, end_style: None,
        }
    }
    pub fn orientation(&self, o: &ScrollbarOrientation) -> Scrollbar {
        let mut s = self.clone(); s.orientation = o.clone(); s
    }
    pub fn thumb_style(&self, st: &Style) -> Scrollbar { let mut s = self.clone(); s.thumb_style = Some(st.clone()); s }
    pub fn track_style(&self, st: &Style) -> Scrollbar { let mut s = self.clone(); s.track_style = Some(st.clone()); s }
    pub fn begin_style(&self, st: &Style) -> Scrollbar { let mut s = self.clone(); s.begin_style = Some(st.clone()); s }
    pub fn end_style(&self, st: &Style) -> Scrollbar { let mut s = self.clone(); s.end_style = Some(st.clone()); s }
    fn __repr__(&self) -> String { "Scrollbar()".to_string() }
}
