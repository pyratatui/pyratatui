...
from pyratatui import (
    Terminal, Gauge, LineGauge, Paragraph,
    Layout, Constraint, Direction,
    Block, Style, Color, Text, Line, Span,
)
...
with Terminal() as term:
    ...
    for step in range(STEPS + 1):
        ...
        def ui(frame, p=pct, s=step):
            ...
            # Info pane
            color = (Color.green() if p < 40 else
                     Color.yellow() if p < 80 else Color.red())
            info = Text([
            ...
            frame.render_widget(
                Paragraph.from_string(f" {p}% — {'█' * (p // 5)}{'░' * (20 - p // 5)}")
                    .style(Style().fg(color)),
                outer[3])
